...
public class DenoisingAutoEncoder0a extends AbstractCnnDenoisingAutoEncoder {
...
//	            .layer(new BatchNormalization())
                ...
                .layer(new ConvolutionLayer.Builder().kernelSize(4, 3).stride(1, 1).activation(Activation.RELU).convolutionMode(ConvolutionMode.Same)
                        .nOut(48).build())
//	            .layer(new BatchNormalization())
                .layer(new SubsamplingLayer.Builder().kernelSize(3, 2).stride(2, 2).poolingType(SubsamplingLayer.PoolingType.MAX).build())
                .layer(new ConvolutionLayer.Builder().kernelSize(4, 3).stride(1, 1).activation(Activation.RELU).convolutionMode(ConvolutionMode.Same)
                        .nOut(48).build())
                .layer(new ConvolutionLayer.Builder().kernelSize(4, 3).stride(1, 1).activation(Activation.RELU).convolutionMode(ConvolutionMode.Same)
                        .nOut(48).build())
