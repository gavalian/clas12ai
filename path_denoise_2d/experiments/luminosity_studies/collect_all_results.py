...
def error_bar_charts(x, m, rms, x_label, y_label, clr):
    # plt.figure()
    plt.ylim([0,1])
    plt.xlabel(x_label)
    plt.ylabel(y_label)
    plt.errorbar(x, m, yerr=rms, fmt='o', color=clr)
...
error_bar_charts(models, all_hits, all_hits_err, "Beam Current(nA)", "Efficiency","b")
error_bar_charts(models, all_rec_noise, all_rec_noise_err, "Beam Current(nA)", "Efficiency","r")
# plt.show()
