...
class CnnDenoisingModel0f(CnnDenoisingModelBase):
    """
    ...
    def build_new_model(self):
        k_model = Sequential()
        k_model.add(tf.keras.layers.Conv2D(28, kernel_size=(4, 3), activation='relu', padding="same",
                                         input_shape=(self.layers, self.sensors_per_layer, 1)))
        k_model.add(tf.keras.layers.Conv2D(28, kernel_size=(4, 3), activation='relu', padding="same"))
        k_model.add(tf.keras.layers.MaxPooling2D((2, 2)))
        k_model.add(tf.keras.layers.Conv2D(28, kernel_size=(3, 3), activation='relu', padding="same"))
        k_model.add(tf.keras.layers.Conv2D(28, kernel_size=(3, 3), activation='relu', padding="same"))
