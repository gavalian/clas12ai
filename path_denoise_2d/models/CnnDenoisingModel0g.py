import numpy as np
from termcolor import colored
import tensorflow as tf
from tensorflow.keras.models import Sequential
from models.CnnDenoisingModelBase import CnnDenoisingModelBase


class CnnDenoisingModel0g(CnnDenoisingModelBase):
    """
    A denoising convolutional neural network Keras regressor.
    """

    def build_new_model(self):
        padding_x = 2

        k_model = Sequential()
        k_model.add(tf.keras.layers.Conv2D(54, kernel_size=(4, 3), activation='relu', padding="same",
                                         input_shape=(self.layers, self.sensors_per_layer + padding_x, 1)))
        k_model.add(tf.keras.layers.Conv2D(54, kernel_size=(4, 3), activation='relu', padding="same"))
        k_model.add(tf.keras.layers.MaxPooling2D((2, 2)))
        k_model.add(tf.keras.layers.Conv2D(64, kernel_size=(3, 3), activation='relu', padding="same"))
        k_model.add(tf.keras.layers.Conv2D(64, kernel_size=(3, 3), activation='relu', padding="same"))
        k_model.add(tf.keras.layers.MaxPooling2D((3, 3)))
        k_model.add(tf.keras.layers.Conv2D(64, kernel_size=(3, 3), activation='relu', padding="same"))
        k_model.add(tf.keras.layers.Conv2D(64, kernel_size=(3, 3), activation='relu', padding="same"))
        k_model.add(tf.keras.layers.UpSampling2D((3, 3)))
        k_model.add(tf.keras.layers.Conv2D(54, kernel_size=(4, 3), activation='relu', padding="same"))
        k_model.add(tf.keras.layers.Conv2D(54, kernel_size=(4, 3), activation='relu', padding="same"))
        k_model.add(tf.keras.layers.UpSampling2D((2, 2)))
        k_model.add(tf.keras.layers.Conv2D(1, kernel_size=(4, 3), activation='sigmoid', padding="same"))
        k_model.summary()

        k_model.compile(optimizer='adam', loss='binary_crossentropy')

        self.model = k_model
